...
def install_chaos():
    ...
    if user_choice == "yes":
        try:
            # Install chaos using Go
            print("[✓] Installing chaos...")
            subprocess.run(["go", "install", "github.com/projectdiscovery/chaos-client/cmd/chaos@latest"], check=True)
            print("[✓] chaos installed successfully!")

        except subprocess.CalledProcessError as e:
            print(f"[✗] Error installing chaos: {e}")
    else:
        print("[✗] Skipping chaos installation.")
        return
...
if "chaos" in missing_tools:
    install_chaos()
    missing_tools.remove("chaos")
